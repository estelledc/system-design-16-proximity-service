...
import pg from 'pg';
import { createHttpServer, listen, closeServer } from './http.js';
import { initializeDatabase, ProximityRepository } from './repository.js';
...
export async function serve(environment = process.env) {
  const pool = new Pool({
    ...
    application_name: 'proximity-lab',
    max: 20,
  ...
  const service = new ProximityService(repository, {
    pageTokenSecret: required(environment, 'PAGE_TOKEN_SECRET'),
  });
  const server = createHttpServer({
    service,
    authSecret: required(environment, 'AUTH_FINGERPRINT_SECRET'),
    crashAfterSearchCommit: environment.CRASH_AFTER_SEARCH_COMMIT === '1',
  });
  const address = await listen(server, {
    host: environment.HOST ?? '127.0.0.1',
    port: environment.PORT ? Number.parseInt(environment.PORT, 10) : 3000,
  });
  process.stdout.write(`${JSON.stringify({ operation: 'server_ready', status: 200, port: address.port })}\n`);
